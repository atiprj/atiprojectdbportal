...
export function DashboardViewer({ url, title, type = "powerbi" }: DashboardViewerProps) {
  ...
  // Modifica l'URL per nascondere i controlli di PowerBI se necessario
  const getCleanUrl = (originalUrl: string) => {
    if (type === "powerbi" && originalUrl.includes("powerbi.com")) {
      // Aggiungi parametri per nascondere alcuni controlli se supportati
      const separator = originalUrl.includes("?") ? "&" : "?"
      return `${originalUrl}${separator}chromeless=1`
    }
    return originalUrl
  }
